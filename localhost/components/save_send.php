...
function saveChatMessage($conn, $senderId, $propertyId, $message)
{
    $receiverId = getPropertyOwnerId($conn, $propertyId);

    $saveChat = $conn->prepare("INSERT INTO `chat` (sender_id, receiver_id, message) VALUES (?, ?, ?)");
    $saveChat->execute([$senderId, $receiverId, $message]);
}

// Fungsi untuk mendapatkan ID pemilik properti
function getPropertyOwnerId($conn, $propertyId)
{
    $selectReceiver = $conn->prepare("SELECT user_id FROM `property` WHERE id = ? LIMIT 1");
    $selectReceiver->execute([$propertyId]);
    $fetchReceiver = $selectReceiver->fetch(PDO::FETCH_ASSOC);

    return $fetchReceiver['user_id'];
}
